...
class TemplateRulesTests(unittest.TestCase):
    ...
    def test_all_pipelines_apply_subtitle_policy_independently(self):
        for filename in ('social.json', 'iqiyi.json', 'info_feed_ad.json'):
            ...
            self.assertTrue(all('，' not in line and '！' not in line for line in rendered))
            self.assertTrue(all(len(line.rsplit('}', 1)[-1]) <= 10 for line in rendered))
